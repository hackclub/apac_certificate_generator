...
const { isProd, auth_people } = require("../data");
...
const on_sign = async ({ body, client, ack }) => {
  ...
  try {
    ...
    await certificate_generator(name, university, gender, month, year, isProd);
    ...
    await add_reaction(client, channel_id, "white_check_mark", messg_ts);
  } catch (err) {
    //this is for testing
    event_error = err;
  } finally {
